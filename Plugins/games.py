...
''')
   ...
   if text == 'زوجتي':
     if r.get(f'{m.from_user.id}:marriedMan:{m.chat.id}{Dev_Zaid}'):
       getUser = c.get_users(int(r.get(f'{m.from_user.id}:marriedMan:{m.chat.id}{Dev_Zaid}')))
       return m.reply(f'{k} كلمي زوجك عايزك: {getUser.mention}')
     else:
       return m.reply(f'{k} انت مش متزوج')
   
   if text == 'زوجي':
     if r.get(f'{m.from_user.id}:marriedWomen:{m.chat.id}{Dev_Zaid}'):
       getUser = c.get_users(int(r.get(f'{m.from_user.id}:marriedWomen:{m.chat.id}{Dev_Zaid}')))
       return m.reply(f'{k} كلم مراتك عايزاك: {getUser.mention}')
     else:
       return m.reply(f'{k} انتي مش متزوجة')
   
   if text == "سورس" or text == "السورس":
       try:
           photo = c.get_messages("djhdkdndkdjdkddkfj", 504).photo.file_id
       except:
           photo = "https://i.is.cc/12FMKKIh.jpg"
       return m.reply_photo(
           photo,
           caption="سورس فارس حماية الكروبات، ارفعه مشرف بكروبك واحميها:",
           reply_markup=InlineKeyboardMarkup(
               [[InlineKeyboardButton("الدخول البوت 💥", url="https://t.me/H0_VF_BOT")]]
           )
       )
