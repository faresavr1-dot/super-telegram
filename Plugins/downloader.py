...
def yt_func(c, m, k, channel):
    ...
    if text == "نسخة اليوتيوب" and m.from_user.id == 6168217372:
        if not ytdb.keys():
            return m.reply("تخزين اليوتيوب فاضي")
        else:
            videos = []
            audios = []
    ...
    if text.startswith('ساوند '):
        ...
        for a in result:
            if count == 5:
                break
            url = a['url']
            buttons.append([InlineKeyboardButton(a['name'], switch_inline_query_current_chat=f'{url}#SOUND')])
            count += 1
        m.reply(f'{k} بحث الساوند ~ {query}', reply_markup=InlineKeyboardMarkup(buttons))
        return True
